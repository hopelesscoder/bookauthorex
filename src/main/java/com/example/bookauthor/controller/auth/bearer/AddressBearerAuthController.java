...
public class AddressBearerAuthController {
	
	private static final Logger log = LoggerFactory.getLogger(AddressBearerAuthController.class);
	...
	private ResponseEntity checkAuthorization(String authorization){
		if(authorization == null || !authorization.startsWith("Bearer")){
			...
			respondeHeaders.add("WWW-Authenticate", "Bearer realm=\"addresses\"");
			return new ResponseEntity<>(respondeHeaders, HttpStatus.UNAUTHORIZED);
		}else{
			if(!authorization.contains("pZCI6ImNiNDA0.NCwiZXhwIjoxNTg0.L0s9ztAShRe")){
				HttpHeaders respondeHeaders = new HttpHeaders();
				respondeHeaders.add("WWW-Authenticate", "Bearer realm=\"addresses\"");
	...
	public ResponseEntity<String> login(@RequestHeader(value="Authorization", required = false) String authorization){
		...
		String response = "{\"access_token\":\"pZCI6ImNiNDA0.NCwiZXhwIjoxNTg0.L0s9ztAShRe\",\"token_type\":\"Bearer\"}";
